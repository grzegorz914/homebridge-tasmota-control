...
class TasmotaDevice {
    ...
    async getDeviceInfo() {
        ...
        try {
            const deviceInfo = await this.axiosInstance(CONSTANS.ApiCommands.Status);
    ...
    async checkDeviceState() {
        ...
        try {
            ...
            const deviceState = await this.axiosInstance(CONSTANS.ApiCommands.PowerStatus);
    ...
    prepareAccessory() {
        ...
        for (let i = 0; i < channelsCount; i++) {
            ...
            tasmotaService.getCharacteristic(Characteristic.On)
                ...
                .onSet(async (state) => {
                    const powerOn = channelsCount === 1 ? CONSTANS.ApiCommands.Power + CONSTANS.ApiCommands.On : CONSTANS.ApiCommands.Power + (i + 1) + CONSTANS.ApiCommands.On;
                    const powerOff = channelsCount === 1 ? CONSTANS.ApiCommands.Power + CONSTANS.ApiCommands.Off : CONSTANS.ApiCommands.Power + (i + 1) + CONSTANS.ApiCommands.Off;
